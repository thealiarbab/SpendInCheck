...
def dashboard_payload(user_id):
    ...
    return {
        "portfolio": {
            "items": money.rows(PNL_FIELDS, holdings),
            "totals": {
                "value": money.serialise(sum((row[6] for row in holdings), start=0)),
                "pnl": money.serialise(sum((row[5] for row in holdings), start=0)),
                "holdings": len(holdings),
            },
        },
        "recent": money.rows(TRANSACTION_FIELDS, recent),
    }
