...
def test_the_suggestions_are_stocksaathis_list(api_account):
    """The credit under the box says "instruments by StockSaathi", and this
    is what makes that true rather than decorative.

    Not a stub: a real row from the real table, asserting the two fields
    that can only have come from their published universe. NSE's own CSV --
    what this used to seed from -- has no sector column at all, and AMFI's
    raw list has no fund house. If somebody reseeds this from the exchange
    again, this test is what notices.
    """
    body = api_account["client"].get(
        "/api/v1/symbols/search?q=reliance").get_json()["items"]
